...
+ (instancetype)wmf_placesActivityWithURL:(NSURL *)activityURL {
    ...
    for (NSURLQueryItem *item in components.queryItems) {
        ...
        if ([item.name isEqualToString: @"WMFPlacesCoordinates"]) {
    ...
    if (placesCoordinates) {
        activity.userInfo = @{@"WMFPlacesCoordinates": placesCoordinates};
    }
...
- (WMFUserActivityType)wmf_type {
    if (self.userInfo[@"WMFPage"] != nil) {
        ...
        return WMFUserActivityTypePlacesWithCoordinates;
    } else {
...
- (NSString *)wmf_placesCoordinates {
    return self.userInfo[@"WMFPlacesCoordinates"];
}
